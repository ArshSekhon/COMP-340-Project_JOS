...
mem_init(void)
{
	...
	// Your code goes here:
	pages = (struct PageInfo*)boot_alloc(npages*sizeof(struct PageInfo));// allocate the array  
    memset(pages, 0, npages*sizeof(struct PageInfo)); // make sure contents are empty
...
page_alloc(int alloc_flags)
{
	...
	struct PageInfo *freed_page = page_free_list;
    if (freed_page == NULL) { 
        return NULL;
    }
    page_free_list = freed_page->pp_link;
    freed_page->pp_link = NULL;
    if (alloc_flags & ALLOC_ZERO) {
        memset(page2kva(freed_page), 0, PGSIZE); //fills the entire returned physical page with '\0' bytes.
    }
    return freed_page;
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	//mmu.h is really important to understand this code
	int page_dir_index = PDX(va);
	int page_table_index = PTX(va);
	if(!*(pgdir+page_dir_index) & PTE_P){ //check if pg directory entry exists
		if(create){
			struct PageInfo *page = page_alloc(1);
			
			if(!page) // if page is NULL then page allocation has failed
				return NULL;
			page->pp_ref++;
			*(pgdir+page_dir_index) = page2pa(page) | PTE_P | PTE_U | PTE_W;
	...
	return (pte_t *) KADDR(PTE_ADDR(*(pgdir+page_dir_index)))+ PTX(va);
}
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	// retrieve page table entry from page table 
	// we donot want to create a new page table entry so we pass 0 
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    if (pp == NULL || !(*page_table_entry & PTE_P)) {   
        return;
    }
    page_decref(pp);     
    *page_table_entry = 0;    
